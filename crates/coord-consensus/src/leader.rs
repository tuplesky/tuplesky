...
impl Leader {
    /// A leader from its configuration and the recovered promise row.
    pub fn new(config: LeaderConfig, durable_promise: Option<PromiseRecordV1>) -> Self {
        let ballots =
    ...
    pub fn from_recovered(
        ...
        let mut last = decision.entries.keys().max().copied();
    ...
    fn on_storage(&mut self, event: &StorageEvent) -> Vec<Effect> {
        ...
        self.learn();
        let mut out = self.deliver_due_report();
    ...
    fn on_peer(&mut self, from: ReplicaId, frame: &[u8]) -> Vec<Effect> {
        ...
        match message {
            ...
            ProtocolMessage::Sync(decision) => {
                ...
                if self.deposed() {
                    self.pending_sync = Some((from, decision));
                }
                Vec::new()
            }

...
impl DurableLedger {
    ...
    /// The batch under `barrier` is durable: its record is now a fact.
    pub fn durable(&mut self, barrier: BarrierId) -> Option<CommandId> {
...
pub struct ReportPage {
    ...
    /// Digest binding replica, ballots, page, total and entries.
    pub digest: Digest32,
}
...
fn page_digest(
    ...
    let body = postcard::to_allocvec(&(replica, ballot, committed, page, total, entries))
        .expect("bounded");
...
pub struct ReportAssembler {
    ...
    totals: BTreeMap<ReplicaId, (u32, Ballot)>,
}
...
impl ReportAssembler {
    ...
    pub fn accept(&mut self, page: ReportPage) -> Result<(), PageError> {
        ...
        match self.totals.get(&page.replica) {
            Some((total, committed))
                if *total != page.total || *committed != page.committed_ballot =>
            {
            ...
            None => {
                self.totals
                    .insert(page.replica, (page.total, page.committed_ballot));
            }
        }
        self.pages
            .entry(page.replica)
    ...
    pub fn complete(&self) -> Vec<RecoveryReport> {
        ...
        for (replica, pages) in &self.pages {
            let Some((total, committed)) = self.totals.get(replica) else {
                continue;
            };
    ...
    pub fn incomplete(&self) -> Vec<ReplicaId> {
        self.pages
            ...
            .filter(|(r, pages)| {
                self.totals
                    .get(r)
                    .is_some_and(|(total, _)| pages.len() != *total as usize)
            })
            .map(|(r, _)| *r)
            .collect()
    }

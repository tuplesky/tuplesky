...
fn kv(f: &Fixture, key: &[u8], ttl: u32) -> KineKv {
    KineKv::project(key, &f.current[key], ttl)
}
...
fn replaced_ttl_fences_the_prior_binding_and_zero_detaches() {
    ...
    assert_eq!(current.mod_revision, rev(2));
    assert_eq!(current.version, 2);
    assert_eq!(f.current[b"/k".as_slice()].lease, Some(B2));
...
#[test]
fn kine_results_carry_no_binding_identity_and_only_private_ttls() {
    let mut f = Fixture::new();
    // A key attached to a native lease has no Kine TTL: the native lease
    // is not a private binding, so a failed comparison reports zero and
    // exposes neither the native nor any private identity.
    f.run(&grant(L1, 30));
    f.run(&put(b"/n", b"1", Some(L1))); // rev 1
    let p = f.run(&kine_update(b"/n", b"2", 9, 0, None));
    assert_eq!(
        p.response.outcome,
        Outcome::KineUpdated {
            updated: false,
            current: Some(kv(&f, b"/n", 0)),
        }
    );
    let p = f.run(&kine_delete(b"/n", Some(9)));
    assert_eq!(
        p.response.outcome,
        Outcome::KineDeleted {
            deleted: false,
            prev: Some(kv(&f, b"/n", 0)),
        }
    );
    // A live private binding reports its TTL.
    f.run(&kine_create(b"/k", b"v1", 60, Some(B1))); // rev 2
    let p = f.run(&kine_update(b"/k", b"v2", 9, 0, None));
    assert_eq!(
        p.response.outcome,
        Outcome::KineUpdated {
            updated: false,
    ...
    // The projection is the whole Kine-facing entry: value and revisions,
    // and nothing that names a lease.
    let seen = kv(&f, b"/k", 60);
    assert_eq!(seen.value, b"v1".to_vec());
    assert_eq!(seen.create_revision, rev(2));
    assert_eq!(seen.mod_revision, rev(2));
    assert_eq!(seen.version, 1);
    let encoded = postcard::to_allocvec(&seen).unwrap();
    assert!(
        !encoded
            .windows(16)
            .any(|w| w == B1.as_bytes() || w == L1.as_bytes()),
        "no lease identity is encoded in a Kine result"
    );
}

#[test]
fn failed_kine_comparisons_are_charged_against_the_response_budget() {
    let mut f = Fixture::new();
    let value = vec![0x42; 4096];
    f.run(&kine_create(b"/big", &value, 0, None)); // rev 1
    f.limits.max_response_bytes = 1024;
    let v = f.view(ALICE);
    // A mismatched update or conditional delete returns the current entry
    // and is bounded like a read.
    assert_eq!(
        plan(&kine_update(b"/big", b"x", 9, 0, None), &v, &f.limits).unwrap_err(),
        PlanError::ResponseTooLarge
    );
    assert_eq!(
        plan(&kine_delete(b"/big", Some(9)), &v, &f.limits).unwrap_err(),
        PlanError::ResponseTooLarge
    );
    // A matching update returns the new entry, which is charged too, while
    // an unconditional delete returns the previous entry as its event.
    assert_eq!(
        plan(&kine_update(b"/big", &value, 1, 0, None), &v, &f.limits).unwrap_err(),
        PlanError::ResponseTooLarge
    );
    assert_eq!(
        plan(&kine_delete(b"/big", None), &v, &f.limits).unwrap_err(),
        PlanError::ResponseTooLarge
    );
    // Under the budget, the same operations plan.
    f.limits.max_response_bytes = 16 * 1024;
    assert!(matches!(
        plan(&kine_update(b"/big", b"x", 9, 0, None), &v, &f.limits)
            .unwrap()
            .response
            .outcome,
        Outcome::KineUpdated { updated: false, .. }
    ));
}

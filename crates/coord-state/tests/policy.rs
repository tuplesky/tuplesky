...
fn permissions_are_branch_specific_fully_contained_and_explicit_for_leases() {
    ...
    // Revoking deletes "a", so the delete rule must cover it as well.
    f.run_internal(&allow(4, ALICE, Action::Delete, KeyInterval::exact(b"a")));
    for (r, action) in [
...
#[test]
fn returning_existing_values_needs_read_and_revocation_needs_delete() {
    let mut f = with_alice();
    f.run(&put(b"a", b"0", None)); // trusted seed, revision 1
    f.run_internal(&allow(1, ALICE, Action::Write, KeyInterval::all()));
    f.run_internal(&allow(2, ALICE, Action::Delete, KeyInterval::all()));
    f.run_internal(&allow(3, ALICE, Action::LeaseAttach, KeyInterval::all()));
    f.run_internal(&allow(4, ALICE, Action::LeaseGrant, KeyInterval::all()));
    f.run_internal(&allow(5, ALICE, Action::LeaseRevoke, KeyInterval::all()));
    // A write-only principal cannot learn what it overwrites or deletes.
    let mut p = PutOp {
        key: b"a".to_vec(),
        value: b"1".to_vec(),
        lease: None,
    ...
    assert_eq!(
        f.run_session(&S1, &req(CanonicalOperation::Put(p.clone())))
            .response
            .outcome,
        Outcome::ErrPermissionDenied
    );
    ...
    assert_eq!(
        f.run_session(&S1, &req(CanonicalOperation::DeleteRange(d.clone())))
            .response
            .outcome,
        Outcome::ErrPermissionDenied
    );
    assert_eq!(f.current[b"a".as_slice()].value, b"0".to_vec());
    // Without the previous value the same operations are permitted.
    p.prev_kv = false;
    assert_eq!(
        f.run_session(&S1, &req(CanonicalOperation::Put(p.clone())))
            .response
    ...
    // With read permission the values come back.
    f.run_internal(&allow(6, ALICE, Action::Read, KeyInterval::exact(b"a")));
    p.prev_kv = true;
    p.value = b"2".to_vec();
    assert!(matches!(
        f.run_session(&S1, &req(CanonicalOperation::Put(p)))
            .response
            .outcome,
        Outcome::Put { prev: Some(_) }
    ));
    assert!(matches!(
        f.run_session(&S1, &kine_delete(b"a", Some(1)))
            .response
            .outcome,
        Outcome::KineDeleted { deleted: false, .. }
    ));
    // Revoking an owned lease deletes its attachments, which needs delete
    // permission over each attached key: narrowing delete to "x" denies
    // the revocation of a lease holding "b".
    f.run_session(&S1, &grant(L1, 30));
    f.run_session(&S1, &put(b"b", b"1", Some(L1)));
    f.run_internal(&allow(2, ALICE, Action::Delete, KeyInterval::exact(b"x")));
    let p = f.run_session(&S1, &revoke(L1));
    assert_eq!(p.response.outcome, Outcome::ErrPermissionDenied);
    assert!(p.mutations.is_empty());
    assert!(f.current.contains_key(b"b".as_slice()));
    f.run_internal(&allow(2, ALICE, Action::Delete, KeyInterval::exact(b"b")));
    assert_eq!(
        f.run_session(&S1, &revoke(L1)).response.outcome,
        Outcome::LeaseRevoked { deleted: 1 }
    );
    assert!(!f.current.contains_key(b"b".as_slice()));
}

#[test]
fn trust_rule_generations_never_move_backward() {
    let mut f = with_alice();
    f.run_internal(&allow(1, ALICE, Action::Write, KeyInterval::all()));
    // Regenerating to 2 revokes S1 (admitted at 1); a stale write of
    // generation 1 is rejected and cannot revalidate it.
    assert_eq!(
        f.run_internal(&trust(true, 2)).response.outcome,
        Outcome::PolicyUpdated
    );
    let p = f.run_internal(&trust(true, 1));
    assert_eq!(p.response.outcome, Outcome::ErrTrustRuleInvalid);
    assert!(p.mutations.is_empty());
    assert_eq!(
        f.run_session(&S1, &put(b"a", b"1", None)).response.outcome,
        Outcome::ErrSessionInvalid
    );
    // Disabling and re-enabling at the current generation still works.
    f.run_internal(&trust(false, 2));
    assert_eq!(
        f.run_internal(&trust(true, 2)).response.outcome,
        Outcome::PolicyUpdated
    );
...
#[test]
fn retained_results_are_reauthorized_under_current_policy() {
    use coord_state::authorize_retained;
    let mut f = with_alice();
    f.run_internal(&allow(1, ALICE, Action::Read, KeyInterval::all()));
    f.run_internal(&allow(2, ALICE, Action::Write, KeyInterval::all()));
    f.run(&put(b"a", b"secret", None));
    let request = get(KeyRange::exact(b"a".to_vec()));
    ...
    assert_eq!(
        authorize_retained(&auth(&f), &NS, &request, &outcome),
        Err(Outcome::ErrPermissionDenied)
    );
    // A retained transaction is checked against the branch that ran.
    ...
    assert_eq!(authorize_retained(&auth(&f), &NS, &txn, &ran), Ok(()));
    let ran = Outcome::Txn {
        succeeded: false,
        results: vec![],
    };
    assert_eq!(
        authorize_retained(&auth(&f), &NS, &txn, &ran),
        Err(Outcome::ErrPermissionDenied)
    );
    // A retired session never reads anything back.
    f.run_internal(&InternalCommand::RetireSession {
        namespace: NS,
        session: S1,
    });
...
fn branch_op() -> impl Strategy<Value = BranchOp> {
    prop_oneof![
        (key(), any::<bool>(), any::<bool>()).prop_map(|(k, l, prev_kv)| BranchOp::Put(PutOp {
            key: k,
            value: b"v".to_vec(),

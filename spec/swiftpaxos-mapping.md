...
| `descPool` / `MaxDescRoutines` / `HISTORY_SIZE` | `[EXT]` bounded capacity refuses new work; unresolved acceptance is never evicted; only executed records retire, and a full table retires them before it refuses anything, so the capacity bounds outstanding work rather than a replica's lifetime | `CommandTable::{with_capacity, reclaim, retire}`, `InitError::Backpressure`, `RetireError` | `tests/graph.rs::{backpressure_refuses_new_work_without_deleting_unresolved_acceptance, a_full_table_reclaims_what_it_executed_and_nothing_else}`, `tests/activation.rs::a_cluster_serves_past_its_table_capacity` |

# Collector contract, revision 2 (task-33, revised by task-c01)
...
**What revision 2 changed.** Revision 1 said fan-out was to every voter
at once and said nothing about a destination that could not take it. In
...
`Cancelled`, `Resolved`, `TimedOut`, `Reconfigured`, `Reoffered`,
`Undisseminated`) with identities in lowercase hex. The two added in
revision 2 appear only when delivery did not go straight through, so the
frozen revision-1 scenario produces the same trace it always did. The Rust reference trace of the frozen scenario is

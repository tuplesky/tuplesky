...
Two gaps that suite found now hold the certification, and both are in the daemon rather than in the adapter. `coordd` does not serve watch streams: `Step::Watch` in `bins/coordd/src/serve.rs` counts the watch and drops the responder, and because the hub keeps the registration that nobody drains, the domain stops answering afterwards. And a connection stops being answered after about sixty requests: a single caller issuing one request at a time completes exactly 61 of 100 against a three-voter domain, repeatably and whatever the deadline, which is an exhaustion rather than a slowdown -- the unary lane admits 64 concurrent streams per connection, and a sequential caller reaches that bound only if the frontend is not releasing what a finished request held. The Kine backend spends fewer requests than that before the watch gap stops the domain, so neither was visible before a harness drove the composition the way an API server does.

...
**Addendum PRs.** Work that lands after a task's PR has been opened for review does not go back into that PR. It goes into a follow-up PR named `task-<id>-<word>`, whose title and body name the original task. This keeps a reviewer's diff stable once they have started on it, at the cost of reading a task in two places. Three exist: `task-43-compose` (composing and serving the daemon, after `task-43`'s preview), `task-j04-matrix` (the crash-matrix row, after `task-j04`) and `task-62-63-fixes` (the three defects the republished matrices exposed, after `task-62` and `task-63`). Where a defect surfaced under one task but belongs to another, the commit message names the task that owns the code.

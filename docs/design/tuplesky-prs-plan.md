...
Three gaps that suite found now hold the certification, and all three are in the daemon rather than in the adapter. `coordd` does not serve watch streams: `Step::Watch` in `bins/coordd/src/serve.rs` counts the watch and drops the responder, so the collector's registration exists and nothing ever delivers on it. A key written under a time to live does not expire.  And concurrent callers are not served across a quorum: two callers issuing one request each at a time against three voters complete 13 of 100 operations, while the same two against a single voter complete 100 of 100 and a single caller against the three complete 200 of 200. Every command is initialized with one conservative conflict key, so the dependency chain over all commands is total; that is the mechanism to look at first, and what has been measured is the shape rather than the proof. A fourth, now fixed here, was a connection that stopped being answered after exactly 61 requests: nothing retired an executed record from the leader's command table, so its capacity bounded how many commands a replica could execute in its lifetime. None of them was visible before a harness drove the composition the way an API server does -- the Kine backend holds one session and one in-flight invocation, and every integration test the project had asked one caller a handful of questions.

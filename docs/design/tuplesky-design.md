...
What the boundary may attest, and what it may not, is a type distinction and not a runtime condition. Every receipt attests its verifier's own cluster and domain, the session it is about, the rule generation and ceiling relied on, a unique single-use receipt identity and the boundary's own admission tick. A receipt that is to *establish* the session it names attests, in addition, the principal the verified credential maps to under configured trust, the trust rule actually used and that credential's validity bound; the second set is a separate shape, so a receipt for submitting work under an existing session cannot carry a principal at all. Establishing needs an ingress with session-issuing authority, which is strictly narrower than one that may relay a client's work: being able to submit is not authority to originate an identity. None of it may be taken from a command payload, and the operation that consumes an establishment carries nothing.

The admission is part of what the accepted command durably is: it is recorded with the payload, and its digest is carried by every acknowledgement and bound beside the command in the dependency record, so no quorum can form across replicas that accepted one command as different session-creation facts. Command identity stays the retry key and the canonical request, so a retry under a rotated credential is the same command and its already accepted facts are not replaced by a freshly minted receipt. Consuming the receipt, creating the session, changing the grants it commits and binding the terminal outcome to the invocation happen in one batch, and no usable credential is released before that batch is durable.

A domain's genesis writes its initial trust rule and permission grants when its store is initialized, identically on every replica and without taking an execution position. A session exists only under a trust rule current policy holds enabled and permission is allow-only, so the first command that would write either would otherwise need a session that needs it. Everything after genesis is ordered replicated administration.

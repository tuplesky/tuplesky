...
Confirmed again later, and the second look narrows what the load has to
be: 2 failures in about 10 full-workspace runs, 0 in 12 runs of the
binary alone, and **0 in 15 runs with four spinning CPU hogs beside
it**. Saturating the cores is not enough; what reproduces it is the
scheduling latency of 138 test binaries running in parallel, which is a
different thing from a busy CPU and is why a `yes` loop does not
substitute for it. Nothing in `crates/coord-transport-sim/` had changed
between the runs that failed and the runs that did not.
...
until a submission places it. A duplicate submission produces *no
effects at all* -- `FollowerRejection::Duplicate` returns an empty
effect list -- so a repeat that lands after the hold expired finds a
voter with nothing to hand over, and that acknowledgement is lost for
good. The hold and the repeat schedule are in different crates and
neither is free to move alone, so `PARKED_HOLD` is now derived from
`OFFER_CEILING_MILLIS` rather than chosen beside it.
...
**Did:** `spec/collector-v1.md` is revision 2, because this is a
